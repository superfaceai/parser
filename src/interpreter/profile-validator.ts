...
export class ProfileValidator implements ProfileVisitor {
  ...
  private output: Record<string, MapInput> = {};
  ...
  validate(
    ...
    const getter = this.visit(input, kind, usecase);
    const [result, errors] = getter(input);
  ...
  checkResultDefinition(
    ...
    if (kind === 'result') {
      this.output[node.kind] = { result: node };
      return true;
    }
    return false;
  }
  ...
  visitEnumDefinitionNode(
    ...
    return (input: unknown): ValidationResult => {
      ...
      for (const value of node.values) {
        this.checkResultDefinition(node, kind);
