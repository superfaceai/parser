...
export type Sublexer<C extends LexerContextType> = (slice: string, ...context: SublexerParamsType<C>) => ParseResult<SublexerReturnType<C>>;

export type SublexerParamsType<C extends LexerContextType> =
  C extends LexerContextType.DEFAULT ? []
  : C extends LexerContextType.JESSIE_SCRIPT_EXPRESSION ? [LexerJessieContext['terminationTokens']]
  : never
;
export type SublexerReturnType<C extends LexerContextType> = 
  C extends LexerContextType.DEFAULT ? DefaultSublexerTokenData
  : C extends LexerContextType.JESSIE_SCRIPT_EXPRESSION ? JessieSublexerTokenData
  : never
;
