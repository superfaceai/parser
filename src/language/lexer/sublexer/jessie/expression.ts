...
const debug = createDebug('superface-parser:script:lexer');

function debugSyntaxKindToString(kind: ts.SyntaxKind): string {
  switch (kind) {
...
export function tryParseJessieScriptExpression(
  ...
  for (;;) {
    ...
    if (debug.enabled) {
      debug(
        'Token %s (%d) at position %d',
        debugSyntaxKindToString(token),
        token,
        SCANNER.getTextPos()
      );
    }
