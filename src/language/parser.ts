...
class Parser {
  ...
  parseName(): NameNode {
    ...
    return {
      ...
      loc: this.loc(token) as Location,
    };
  }
  ...
  parseDocument(): DocumentNode {
    ...
    return {
      ...
      loc: this.loc(start) as Location,
    };
  }

  parseDictionary(): DictionaryNode {
    const start = this._lexer.token;
    ...
    if (this.peek(TokenKind.EOF) || this.peek(TokenKind.BRACE_R)) {
      return {
        kind: 'DictionaryNode',
        statements,
        loc: this.loc(start) as Location,
  ...
  parseStatement(): StatementNode {
    const start = this._lexer.token;
    const identifiers: NameNode[] = [];

    while (this.peek(TokenKind.NAME)) {
      identifiers.push(this.parseName());
    }

    if (this.peek(TokenKind.COLON)) {
      this._lexer.advance();

      const value = this.parseValueLiteral(true);
      ...
      this._lexer.advance();
      const value = this.parseDictionary();

      if (this.peek(TokenKind.BRACE_R)) {
        this._lexer.advance();
      } else {
        throw this.unexpected();
      }
  ...
  parseDefinition(): DefinitionNode {
    ...
    if (this.peek(TokenKind.NAME)) {
      ...
      //   if (this.peek(TokenKind.BRACE_R)) {
      //     this._lexer.advance();
      //   } else if (this.peek(TokenKind.EOF)) {
      //   } else {
      //     throw this.unexpected();
      //   }

      //   return definition;
      // } else if (this.peek(TokenKind.BRACE_R)) {
      //   const value = this.parseValueLiteral(true);
  ...
  parseVariableDefinitions(): Array<VariableDefinitionNode> {
    return this.optionalMany(
      TokenKind.PAREN_L,
      this.parseVariableDefinition.bind(this),
      TokenKind.PAREN_R
    );
  }
  ...
  parseArguments(isConst: boolean): Array<ArgumentNode> {
    const item = isConst
      ? this.parseConstArgument.bind(this)
      : this.parseArgument.bind(this);
  ...
  parseFragmentDefinition(): FragmentDefinitionNode {
    ...
    if (this._options && this._options.experimentalFragmentVariables === true) {
      return {
    ...
    switch (token.kind) {
      ...
      case TokenKind.NAME:
        ...
        switch (token.value) {
          ...
          case 'null':
            return { kind: Kind.NULL, loc: this.loc(token) as Location };
          default:
            return {
              kind: Kind.ENUM,
              value: token.value as string,
  ...
  parseList(isConst: boolean): ListValueNode {
    const start = this._lexer.token;
    const item = (): ValueNode => this.parseValueLiteral(isConst);
  ...
  parseObject(isConst: boolean): ObjectValueNode {
    const start = this._lexer.token;
    const item = (): ObjectFieldNode => this.parseObjectField(isConst);
  ...
  parseTypeReference(): TypeNode {
    ...
    if (this.expectOptionalToken(TokenKind.BANG)) {
      return {
        ...
        type: type as ListTypeNode | NamedTypeNode,
        loc: this.loc(start) as Location,
      };
    }
  ...
  parseSchemaDefinition(): SchemaDefinitionNode {
    ...
    const operationTypes = this.many(
      TokenKind.BRACE_L,
      this.parseOperationTypeDefinition.bind(this),
      TokenKind.BRACE_R
    );

    return {
      kind: Kind.SCHEMA_DEFINITION,
      description: description as StringValueNode,
      directives,
      operationTypes,
      loc: this.loc(start) as Location,
    };
  ...
  parseFieldsDefinition(): Array<FieldDefinitionNode> {
    ...
    return this.optionalMany(
      TokenKind.BRACE_L,
      this.parseFieldDefinition.bind(this),
      TokenKind.BRACE_R
    );
  }
  ...
  parseArgumentDefs(): Array<InputValueDefinitionNode> {
    return this.optionalMany(
      TokenKind.PAREN_L,
      this.parseInputValueDef.bind(this),
      TokenKind.PAREN_R
    );
  }
  ...
  parseEnumValuesDefinition(): Array<EnumValueDefinitionNode> {
    return this.optionalMany(
      TokenKind.BRACE_L,
      this.parseEnumValueDefinition.bind(this),
      TokenKind.BRACE_R
    );
  }
  ...
  parseDirectiveLocation(): NameNode {
    const start = this._lexer.token;
    const name = this.parseName();
    if (name.value in DirectiveLocation) {
      return name;
    }
  ...
  loc(startToken: Token): Location | void {
    if (this._options && this._options.noLocation !== true) {
      return new Location(
        startToken,
   ...
   */
  expectOptionalToken(kind: TokenKind): Token | undefined {
    const token = this._lexer.token;
    if (token.kind === kind) {
      this._lexer.advance();
   ...
   * is encountered.
   */
  unexpected(atToken?: Token): Error {
    console.log(this);
