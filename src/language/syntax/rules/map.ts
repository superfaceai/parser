...
export const OPERATION_DEFINITION: SyntaxRuleSrc<OperationDefinitionNode> = documentedNode(SyntaxRule.identifier('operator')
...
export const MAP_DEFINITION: SyntaxRuleSrc<MapDefinitionNode> = documentedNode(SyntaxRule.identifier('map')
  .followedBy(SyntaxRule.identifier())
  .andBy(SyntaxRule.separator('{'))
  .andBy(SyntaxRule.separator('}'))
  .map(
...
export const MAP: SyntaxRuleSrc<MapNode> = documentedNode(
  PROFILE_ID.followedBy(PROVIDER_ID).map(
    (matches): SrcNode<MapNode> => {
      let [profileId, provider] = matches;
