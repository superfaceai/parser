...
import { ASTKindToNode, ASTNode, isNode } from './ast';
...
export type Visitor<
  KindToNode extends object,
  Nodes = KindToNode[keyof KindToNode]
> =
  | EnterLeave<
      | VisitFn<Nodes>
      | ShapeMap<KindToNode, <Node>(node: Node) => VisitFn<Nodes, Node>>
    >
  | ShapeMap<
      KindToNode,
      <Node>(
...
export function visit(
  ...
  let keys: ASTNode[] = [root];
  let index = -1;
  let edits: [any, any][] | undefined = [];
  let node: ASTNode | ASTNode[] | undefined = undefined;
  ...
  do {
    ...
    const isEdited = isLeaving && edits && edits.length !== 0;
    if (isLeaving) {
      key = ancestors.length === 0 ? undefined : path[path.length - 1];
      node = parent;
      parent = ancestors.pop();
      if (isEdited) {
        if (inArray && Array.isArray(node)) {
          node = node?.slice();
        ...
        if (edits) {
          for (let ii = 0; ii < edits.length; ii++) {
            ...
            if (inArray && Array.isArray(node) && editValue === null) {
              node?.splice(editKey, 1);
              editOffset++;
            } else {
              (node as any)[editKey] = editValue;
            }
          }
        }
      }
      ...
      keys = stack?.keys;
      edits = stack?.edits;
      inArray = stack?.inArray ?? false;
      stack = stack?.prev as any;
    } else {
    ...
    if (!Array.isArray(node)) {
      ...
      if (visitFn) {
        ...
        } else if (result !== undefined && edits) {
    ...
    if (result === undefined && isEdited && edits) {
      edits.push([key, node]);
    }
    ...
    if (isLeaving) {
      ...
      keys = inArray ? node : (visitorKeys as any)[(node as any).kind] ?? [];
      index = -1;
      edits = [];
      if (parent) {
    ...
    enter(node, ...args): any {
      for (let i = 0; i < visitors.length; i++) {
        if (skipping[i] == null) {
          ...
          if (fn) {
            const result = fn.apply(visitors[i], [node, ...args]);
            if (result === false) {
              skipping[i] = node;
